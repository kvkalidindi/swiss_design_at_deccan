...
def emit_document_template() -> Path:
    """Render the canonical placeholder template into the published artifact.

    Source: data/templates/document.template.html — placeholder with
    {{LOGO_DATA_URI}} marker.

    Output: skill/assets/templates/document.html — the committed/published
    artifact with the logo base64 inlined so consumers fetching the raw
    GitHub URL get a fully self-contained file (no second fetch needed for
    the binary PNG).
    """
    src = ROOT / "data" / "templates" / "document.template.html"
...
3. Stable raw URL: `{TEMPLATE_RAW_URL_BASE}/templates/document.html` — public, served by GitHub's CDN. The brand logo is already inlined as a data URI in this file.
